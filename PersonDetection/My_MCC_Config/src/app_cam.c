...
/* ----------------------------------------------------------------------------
 * rgb565_to_modelinput_128x160 - 160x120 RGB565 -> 128x160 RGB int8 (HWC)
 *
 * Source : panda_scaled_data, FRAME_BYTES = 160*120*2 little-endian RGB565.
 * Dest   : TinyEngine input buffer (e.g. getInput()), MODEL_IN_BYTES.
 *
 * Y axis is upscaled 120 -> 128 by nearest neighbor: src_y = (dst_y*120)/128.
 * X axis is 1:1 (model width also 160).
 *
 * Per pixel: extract 5/6/5 -> 8/8/8, subtract 128 to map [0,255] -> [-128,127].
 * Layout in dst: dst[((row*MODEL_IN_W) + col)*3 + {0:R,1:G,2:B}].
 * --------------------------------------------------------------------------*/
void rgb565_to_modelinput_128x160(const uint8_t *src, signed char *dst)
{
    for (int dst_y = 0; dst_y < MODEL_IN_H; dst_y++)
    {
        int src_y = (dst_y * IMG_HEIGHT) / MODEL_IN_H;          /* 120/128 */
        ...
        for (int x = 0; x < MODEL_IN_W; x++)
        {
            ...
            out_row[x*3 + 0] = (signed char)((int)r8 - 128);
            out_row[x*3 + 1] = (signed char)((int)g8 - 128);
            out_row[x*3 + 2] = (signed char)((int)b8 - 128);
        }
    }
}
