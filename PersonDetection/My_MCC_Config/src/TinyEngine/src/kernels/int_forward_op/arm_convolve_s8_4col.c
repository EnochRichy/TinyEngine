    ...
    for (i_batch = 0; i_batch < input_batches; i_batch++)
    {
        ...
        for (i_out_y = 0; i_out_y < output_y; i_out_y++)
        {
            for (i_out_x = 0; i_out_x < output_x; i_out_x++)
            {
                ...
                /* Computation is fired for every 2 columns */
                if (four_column_buf == buffer_a + 2 * input_ch * kernel_y * kernel_x)
                {
                    const int32_t num_col_a = input_ch * kernel_y * kernel_x;
                    const int32_t aligned_num_col_a = (num_col_a + 3) & ~3;
                    out =
