...
signed char* getOutput() {
    ...
    def _parseoffsetBias(self, Lindex, bias, input_offset, weight, channel, bias_name=None, is_const=True):
        ...
        fp.write(string)
        kernelsize = int(len(weight) / channel)
        input_offset_py = int(input_offset)
        # fuse the offset into bias
        for i in range(channel):
            tmpW = 0
            for j in range(kernelsize):
                tmpW += int(weight[j * channel + i])
            fp.write(str(self.int32_clip(int(bias[i]) + tmpW * input_offset_py)) + ", ")
        fp.write("};\n")
        ...
        for i in range(channel):
            ...
            for j in range(kernelsize):
                tmpW += int(weight[j * channel + i])
            fp.write(str(int(bias[i]) + tmpW * input_offset_py - self.int32_clip(int(bias[i]) + tmpW * input_offset_py)) + ", ")
        fp.write("};\n")
